...
class Apostille
{
    ...
    {
        if (!in_array($net, ['mainnet', 'testnet'], true)) {
    ...
    public function setting($filename, $type = 'public', $algo = 'sha256', $net = 'mainnet')
    {
        ...
        if (!in_array($algo, ['md5', 'sha1', 'sha256', 'sha3-256', 'sha3-512'], true)) {
    ...
    public function Run()
    {
        ...
        switch ($this->algo) {
            ...
            case 'sha3-512' :
                ...
                break;
            default:
                throw new Exception ("Error:未対応の暗号方式です。");
        }
        if ($algo < 4) {
    ...
    public function Check($baseurl = 'http://localhost:7890')
    {
        ...
        if (preg_match('/^fe4e5459([08])([0-9abcdef])([0-9abcdef]*)/', $txdata['transaction']['message']['payload'],
            $matches)) {
            $type = $matches[1];
            $hash = $matches[3];
            switch ($matches[2]) {
        ...
        if ($this->payload === $txdata['transaction']['message']['payload']) {
            return ['status' => true, 'code' => 0, 'detail' => $txdata['transaction']];
        }
    }
